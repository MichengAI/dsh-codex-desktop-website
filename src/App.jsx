...
const translations = {
  ...
  "Codex UI 与": "Codex UI and", "八个随附插件。": "eight bundled plugins.",
  "DSH Codex Desktop 将 Codex UI、专家预设、技能管理、会话归档、IM Connect、自动化、上下文管理和增强侧边栏带到桌面端。": "DSH Codex Desktop brings Codex UI, expert presets, skill management, conversation archives, IM Connect, automation, context management, and an enhanced sidebar to the desktop.",
  "下载桌面端": "Download desktop app", "查看随附组件": "View bundled plugins", "支持系统": "Platforms", "运行方式": "Runtime", "本机服务": "Local service", "更新入口": "Updates",
  "DSH Codex Desktop 产品总览": "DSH Codex Desktop product overview", "放大查看 DSH Codex Desktop 产品总览": "Open DSH Codex Desktop product overview", "Codex UI 与八个随附插件": "Codex UI and eight bundled plugins",
  ...
  "插件集合开箱即用": "Plugins are ready to use", "八个功能插件随首次启动准备完成，按你的工作方式选择并持续扩展。": "Eight functional plugins are prepared on first launch. Choose what fits your workflow and extend it over time.",
  ...
  "组件": "PLUGINS", "随附插件": "Bundled plugins", "八个独立插件，": "Eight independent plugins", "覆盖常见工作环节。": "for everyday work.",
    ...
    <main id="top">
      <section className="hero page-width" aria-labelledby="hero-title">
        <div>
          ...
          <h1 id="hero-title">Codex UI 与<br /><em>八个随附插件。</em></h1>
          <p className="lead">DSH Codex Desktop 将 Codex UI、专家预设、技能管理、会话归档、IM Connect、自动化、上下文管理和增强侧边栏带到桌面端。</p>
        ...
        <figure className="hero-product"><button className="screenshot-trigger" type="button" onClick={() => setActiveScreenshot({ src: "/assets/product-overview.png", alt: "DSH Codex Desktop 产品总览" })} aria-label="放大查看 DSH Codex Desktop 产品总览"><img src="/assets/product-overview.png" alt="DSH Codex Desktop 产品总览" /></button><figcaption><span>DSH CODEX DESKTOP</span><span>Codex UI 与八个随附插件</span></figcaption></figure>
      </section>
      ...
      <section className="desktop page-width" id="desktop" aria-labelledby="desktop-title">
        ...
        <div className="desktop-grid"><div className="preview"><button className="screenshot-trigger" type="button" onClick={() => setActiveScreenshot({ src: "https://raw.githubusercontent.com/MichengAI/dsh-codex-desktop/main/assets/screenshots/desktop-conversation-dark.png", alt: "DSH Codex Desktop 深色会话工作区" })} aria-label="放大查看深色会话工作区"><img src="https://raw.githubusercontent.com/MichengAI/dsh-codex-desktop/main/assets/screenshots/desktop-conversation-dark.png" alt="DSH Codex Desktop 深色会话工作区" /></button></div><div className="capabilities"><article><b>01</b><div><h3>Codex UI 是主界面</h3><p>围绕项目、任务与会话组织日常 Agent 工作，而不是单独启动一个底层运行时。</p></div></article><article><b>02</b><div><h3>插件集合开箱即用</h3><p>八个功能插件随首次启动准备完成，按你的工作方式选择并持续扩展。</p></div></article><article><b>03</b><div><h3>桌面端负责交付</h3><p>内置本地运行环境并保留用户数据，让工作台可直接安装、更新和使用。</p></div></article></div></div>
      ...
      <section className="plugins page-width" id="plugins" aria-labelledby="plugins-title"><div className="intro plugin-intro"><div><Kicker number="组件">随附插件</Kicker><h2 id="plugins-title">八个独立插件，<br />覆盖常见工作环节。</h2></div><p>这些组件会在首次启动时准备到 DSH profile 中。每个插件都有自己的仓库、版本和更新路径；桌面端运行时不会被插件更新覆盖。</p></div><div className="plugin-grid">{plugins.map(([name, packageName, description, href], index) => <a className="plugin" key={packageName} href={href} target="_blank" rel="noreferrer"><div><small>{String(index + 1).padStart(2, "0")}</small><Arrow /></div><h3>{name}</h3><p>{description}</p><code>{packageName}</code></a>)}<a className="plugin plugin-market" href="https://dshmarket.com" target="_blank" rel="noreferrer"><div><small>插件市场</small><Arrow /></div><h3>dshmarket</h3><p>随桌面端提供，用于发现、安装、更新、启用和诊断其他 DSH 插件。</p><code>dshmarket</code></a></div></section>
